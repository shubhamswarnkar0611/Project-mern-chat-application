        ...
        <>
            <Accordion defaultActiveKey="0">
                <Accordion.Item eventKey="0">
                    <Accordion.Header>Available rooms</Accordion.Header>
                    <Accordion.Body>
                        <ListGroup>
                            {rooms.map((room, idx) => (
                ...
                <Accordion.Item eventKey="1">
                    ...
                    <Accordion.Body>
                        <input type="text" className="form-control mb-2" placeholder="Search Members" onKeyUp={(e) => handleSearchMembers(e)} />
                        {members.map((member) => (
                            ...
                            <ListGroup.Item key={member.id} style={{ cursor: "pointer" }} active={privateMemberMsg?._id == member?._id} onClick={() => handlePrivateMemberMsg(member)} disabled={member._id === user._id}>
                                <Row>
                                    <Col xs={2} className="member-status">
                                        <img alt="" src={member.picture} className="member-status-img" />
